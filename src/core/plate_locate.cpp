...
int CPlateLocate::plateLocate(Mat src, vector<Mat>& resultVec)
{
	...
	//Mat element = getStructuringElement(MORPH_RECT, Size(m_MorphSizeWidth * 2, m_MorphSizeHeight * 2) );
	//morphologyEx(img_threshold, img_threshold, MORPH_CLOSE * 2, element);
...
int CPlateLocate::plateLocate2(Mat src, vector<Mat>& resultVec)
{
	...
	{
		stringstream ss(stringstream::in | stringstream::out);
		ss << "image/tmp/debug_channel" << ".jpg";
