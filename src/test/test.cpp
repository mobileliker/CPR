...
const string testOption[] = 
	{
		"1. test plate_locate2(车牌定位);"		/* 车牌定位 */,
		"2. test plate_judge(车牌判断);"		/* 车牌判断 */,  
		"3. test plate_detect2(车牌检测);"		/* 车牌检测（包含车牌定位与车牌判断） */, 
		"4. test chars_segment2(字符分隔);"		/* 字符分隔 */, 
		"5. test chars_identify2(字符鉴别);"		/* 字符鉴别 */,  
		"6. test chars_recognise2(字符识别);"		/* 字符识别（包含字符分隔与字符鉴别） */,
		"7. test plate_recognize2(车牌识别);"		/* 车牌识别 */, 
...
int test_chars_identify()
{
	...
	int result = cs.charsSegment2(src, resultVec);
	if (result == 0)
	{
		int num = resultVec.size();
		for (int j = 0; j < num; j++)
	...
	const string plateLicense = "粤A0069S";
...
int test_plate_detect()
{
	...
	int result = pd.plateDetect2(src, resultVec);
	if (result == 0)
	{
		int num = resultVec.size();
		for (int j = 0; j < num; j++)
...
int test_chars_recognise()
{
	...
	int result = cr.charsRecognise2(src, charsRecognise);
	if (result == 0)
	{
